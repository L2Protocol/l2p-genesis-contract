...
contract L2PRegistrarController is
    Ownable,
    ...
    ERC20Recoverable
{
    ...
    // @notice The node (i.e. namehash) for the l2p TLD.
    bytes32 private constant L2P_NODE =
    ...
    // @notice The base registrar implementation for the l2p TLD.
    ...
    /// @notice The price oracle for the l2p TLD.
    ...
    /// @notice Constructor for the L2PRegistrarController.
    ///
    /// @param _base The base registrar implementation for the l2p TLD.
    /// @param _prices The price oracle for the l2p TLD.
    ...
    function register(
        ...
        if (registration.resolver == address(0)) {
            ...
            bytes32 namehash = keccak256(abi.encodePacked(L2P_NODE, labelhash));
            ens.setRecord(
                namehash,
                registration.owner,
            ...
            if (registration.reverseRecord & REVERSE_RECORD_ETHEREUM_BIT != 0)
                reverseRegistrar.setNameForAddr(
                    ...
                    msg.sender,
                    registration.resolver,
                    string.concat(registration.label, ".l2p")
                );
